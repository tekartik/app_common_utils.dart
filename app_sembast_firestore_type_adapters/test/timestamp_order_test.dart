...
Future<List<String>> getSnapshotKeys(
  QueryRef<String, dynamic> queryRef,
  ...
  group('timestamp', () {
    ...
    test('order', () async {
      ...
      Future checkContent() async {
        ...
        expect(
          (await getSnapshotKeys(store.query(finder: reverseFinder), db)),
          ['test2', 'test1', 'test4', 'test3'],
        );

        expect((await store.findFirst(db, finder: finder))!.key, 'test3');
        expect(
          (await store.findFirst(db, finder: reverseFinder))!.key,
          'test2',
        );
      }
