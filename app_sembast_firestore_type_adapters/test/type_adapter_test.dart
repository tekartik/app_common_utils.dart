...
void main() {
  group('type_adapters', () {
    test('Timestamp', () {
      expect(
        sembastFirestoreTimestampAdapter.encode(
      ...
      expect(
        sembastFirestoreTimestampAdapter.decode({
          'seconds': 0,
          'nanoseconds': 1000000,
        }),
        Timestamp.fromMillisecondsSinceEpoch(1),
    ...
    test('Blob', () {
      expect(
        sembastFirestoreBlobAdapter.encode(Blob(Uint8List.fromList([1, 2, 3]))),
        'AQID',
      );

      expect(
        sembastFirestoreBlobAdapter.decode('AQID'),
    ...
    test('GeoPoint', () {
      ...
      expect(
        sembastFirestoreGeoPointAdapter.decode({'latitude': 1, 'longitude': 2}),
        const GeoPoint(1, 2),
      );
      expect(
        sembastFirestoreGeoPointAdapter.decode({
          'latitude': 1.1,
          'longitude': 2.2,
        }),
